...
MODE_TO_VERB = {1: RequestVerb.GET, 2: RequestVerb.WRITE, 3: RequestVerb.DELETE}
...
class RKSOKPhoneBook:
    ...
    def _parse_response(self, raw_response: str) -> str:
        ...
        return (
            HUMAN_READABLE_ANSWERS.get(self._verb)
            .get(response_status)
...
def run_client() -> None:
    ...
    try:
        ...
        process_critical_exception(
            "Не могу подключиться к указанному " "серверу и порту"
        )
    ...
    try:
        ...
        process_critical_exception("Не смог разобрать ответ от сервера РКСОК:(")
    print(
